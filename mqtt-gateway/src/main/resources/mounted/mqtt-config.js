    ...
    return (react_js_default().createElement("div", { className: "broker-settings multi-broker" },
        ...
        react_js_default().createElement("div", { className: "broker-edit-panel" }, editingBroker ? (react_js_default().createElement("form", { onSubmit: handleSave },
            ...
            react_js_default().createElement("div", { className: "form-section" },
                ...
                react_js_default().createElement("div", { className: "form-row" },
                    react_js_default().createElement("div", { className: "form-group" },
                        react_js_default().createElement("label", { htmlFor: "slowReconnectIntervalSeconds" }, "Slow Reconnect Interval (seconds)"),
                        react_js_default().createElement("input", { type: "number", id: "slowReconnectIntervalSeconds", name: "slowReconnectIntervalSeconds", value: editingBroker.slowReconnectIntervalSeconds, onChange: handleChange, min: 30, max: 86400 }),
                        react_js_default().createElement("small", null, "Used after max reconnect attempts (e.g., 600 = 10 minutes)"))),
                react_js_default().createElement("div", { className: "form-row" },
...
const TagSelection = ({ config, onConfigSaved }) => {
    ...
    const normalizePayloadFields = (fields) => {
        return {
    ...
    const normalizeTopicMapping = (mapping) => {
    ...
    const updatePayloadField = (key, enabled) => {
        setFormData(prev => ({
            ...prev,
            payloadFields: {
    ...
    const updatePayloadCoreField = (key, enabled) => {
        setFormData(prev => ({
            ...prev,
            payloadFields: {
                ...normalizePayloadFields(prev.payloadFields),
    ...
    const updateMappingPayloadMode = (id, useDefault) => {
        setFormData(prev => ({
            ...prev,
    ...
    const updateMappingPayloadCoreField = (id, key, enabled) => {
        setFormData(prev => ({
            ...prev,
            topicMappings: prev.topicMappings.map(mapping => {
                if (mapping.id !== id) {
    ...
    const updateMappingPayloadField = (id, key, enabled) => {
        setFormData(prev => ({
            ...prev,
            topicMappings: prev.topicMappings.map(mapping => {
                if (mapping.id !== id) {
    ...
    const updateMappingPublishMode = (id, publishMode) => {
        setFormData(prev => ({
            ...prev,
            topicMappings: prev.topicMappings.map(mapping => (mapping.id === id ? { ...mapping, publishMode } : mapping))
        }));
    };
    const updateMappingPreserveCase = (id, preserveTopicCase) => {
        setFormData(prev => ({
            ...prev,
            topicMappings: prev.topicMappings.map(mapping => (mapping.id === id ? { ...mapping, preserveTopicCase } : mapping))
        }));
    };
    const updateMappingBatchWindow = (id, batchWindowMs) => {
        setFormData(prev => ({
            ...prev,
            topicMappings: prev.topicMappings.map(mapping => (mapping.id === id ? { ...mapping, batchWindowMs } : mapping))
        }));
    };
    const updateMappingMaxBatchSize = (id, maxBatchSize) => {
        setFormData(prev => ({
            ...prev,
            topicMappings: prev.topicMappings.map(mapping => (mapping.id === id ? { ...mapping, maxBatchSize } : mapping))
        }));
    };
    ...
    const renderPayloadFields = (fields, namePrefix, onCoreChange, onPropertyChange) => (react_js_default().createElement((react_js_default()).Fragment, null,
        react_js_default().createElement("small", null, "Value and timestamp are always included in the payload."),
        react_js_default().createElement("div", { className: "payload-core" },
            react_js_default().createElement("div", { className: "payload-core-row" },
                react_js_default().createElement("span", null, "Quality"),
                react_js_default().createElement("div", { className: "payload-radio-group" },
                    react_js_default().createElement("label", null,
                        react_js_default().createElement("input", { type: "radio", name: `${namePrefix}-includeQuality`, checked: fields.includeQuality, onChange: () => onCoreChange('includeQuality', true) }),
                        "Include"),
                    react_js_default().createElement("label", null,
                        react_js_default().createElement("input", { type: "radio", name: `${namePrefix}-includeQuality`, checked: !fields.includeQuality, onChange: () => onCoreChange('includeQuality', false) }),
                        "Exclude"))),
            react_js_default().createElement("div", { className: "payload-core-row" },
                react_js_default().createElement("span", null, "Quality Code"),
                react_js_default().createElement("div", { className: "payload-radio-group" },
                    react_js_default().createElement("label", null,
                        react_js_default().createElement("input", { type: "radio", name: `${namePrefix}-includeQualityCode`, checked: fields.includeQualityCode, onChange: () => onCoreChange('includeQualityCode', true) }),
                        "Include"),
                    react_js_default().createElement("label", null,
                        react_js_default().createElement("input", { type: "radio", name: `${namePrefix}-includeQualityCode`, checked: !fields.includeQualityCode, onChange: () => onCoreChange('includeQualityCode', false) }),
                        "Exclude"))),
            react_js_default().createElement("div", { className: "payload-core-row" },
                react_js_default().createElement("span", null, "Tag Path"),
                react_js_default().createElement("div", { className: "payload-radio-group" },
                    react_js_default().createElement("label", null,
                        react_js_default().createElement("input", { type: "radio", name: `${namePrefix}-includeTagPath`, checked: fields.includeTagPath, onChange: () => onCoreChange('includeTagPath', true) }),
                        "Include"),
                    react_js_default().createElement("label", null,
                        react_js_default().createElement("input", { type: "radio", name: `${namePrefix}-includeTagPath`, checked: !fields.includeTagPath, onChange: () => onCoreChange('includeTagPath', false) }),
                        "Exclude")))),
        react_js_default().createElement("div", { className: "payload-properties" }, payloadPropertyGroups.map(group => (react_js_default().createElement("details", { key: `${namePrefix}-${group.id}`, className: "payload-group", open: group.id === 'basic' },
            react_js_default().createElement("summary", { className: "payload-group-summary" }, group.label),
            react_js_default().createElement("div", { className: "payload-group-fields" }, group.fields.map(field => {
                const enabled = fields.properties[field.key] === true;
                return (react_js_default().createElement("div", { key: `${namePrefix}-${field.key}`, className: "payload-property-row" },
                    react_js_default().createElement("span", null, field.label),
                    react_js_default().createElement("div", { className: "payload-radio-group" },
                        react_js_default().createElement("label", null,
                            react_js_default().createElement("input", { type: "radio", name: `${namePrefix}-payload-${field.key}`, checked: enabled, onChange: () => onPropertyChange(field.key, true) }),
                            "Include"),
                        react_js_default().createElement("label", null,
                            react_js_default().createElement("input", { type: "radio", name: `${namePrefix}-payload-${field.key}`, checked: !enabled, onChange: () => onPropertyChange(field.key, false) }),
                            "Exclude"))));
            }))))))));
    const renderMappingItem = (mapping) => {
        ...
        return (react_js_default().createElement("div", { key: mappingId, className: `mapping-item ${!mapping.enabled ? 'disabled' : ''}` },
            ...
            react_js_default().createElement("div", { className: "mapping-payload" },
                react_js_default().createElement("div", { className: "mapping-topic-options" },
                    react_js_default().createElement("div", { className: "mapping-option-row" },
                        react_js_default().createElement("span", null, "Topic Mode"),
                        react_js_default().createElement("div", { className: "payload-radio-group" },
                            react_js_default().createElement("label", null,
                                react_js_default().createElement("input", { type: "radio", name: `publish-mode-${mappingId}`, checked: publishMode === 'PER_TAG_TOPIC', onChange: () => updateMappingPublishMode(mapping.id, 'PER_TAG_TOPIC') }),
                                "Per tag topic"),
                            react_js_default().createElement("label", null,
                                react_js_default().createElement("input", { type: "radio", name: `publish-mode-${mappingId}`, checked: publishMode === 'SINGLE_TOPIC', onChange: () => updateMappingPublishMode(mapping.id, 'SINGLE_TOPIC') }),
                                "Single topic"))),
                    react_js_default().createElement("label", { className: "mapping-checkbox" },
                        react_js_default().createElement("input", { type: "checkbox", checked: preserveTopicCase, onChange: (e) => updateMappingPreserveCase(mapping.id, e.target.checked) }),
                        "Preserve tag case in topic segments"),
                    publishMode === 'SINGLE_TOPIC' && (react_js_default().createElement("div", { className: "mapping-batch-settings" },
                        react_js_default().createElement("small", null, "Publishes all matching tags to the topic prefix."),
                        react_js_default().createElement("div", { className: "mapping-option-row" },
                            react_js_default().createElement("label", { htmlFor: `batch-window-${mappingId}` }, "Batch window (ms)"),
                            react_js_default().createElement("input", { id: `batch-window-${mappingId}`, type: "number", min: 0, step: 10, value: batchWindowMs ?? '', placeholder: "100", onChange: (e) => {
                        ...
                        react_js_default().createElement("div", { className: "mapping-option-row" },
                            react_js_default().createElement("label", { htmlFor: `batch-size-${mappingId}` }, "Max batch size"),
                            react_js_default().createElement("input", { id: `batch-size-${mappingId}`, type: "number", min: 1, step: 1, value: maxBatchSize ?? '', placeholder: "10", onChange: (e) => {
                                    const raw = e.target.value;
                                    if (raw === '') {
                        ...
                        react_js_default().createElement("small", null, "0 ms disables batching (publishes immediately).")))),
                react_js_default().createElement("div", { className: "mapping-payload-header" },
                    react_js_default().createElement("span", null, "Payload Fields"),
                    react_js_default().createElement("div", { className: "payload-radio-group" },
                        react_js_default().createElement("label", null,
                            react_js_default().createElement("input", { type: "radio", name: `payload-mode-${mappingId}`, checked: useDefaultPayload, onChange: () => updateMappingPayloadMode(mapping.id, true) }),
                            "Use default"),
                        react_js_default().createElement("label", null,
                            react_js_default().createElement("input", { type: "radio", name: `payload-mode-${mappingId}`, checked: !useDefaultPayload, onChange: () => updateMappingPayloadMode(mapping.id, false) }),
                            "Custom"))),
                !useDefaultPayload && renderPayloadFields(mappingFields, `mapping-${mappingId}`, (key, enabled) => updateMappingPayloadCoreField(mapping.id, key, enabled), (key, enabled) => updateMappingPayloadField(mapping.id, key, enabled)))));
    };
        ...
        react_js_default().createElement("form", { onSubmit: handleSave },
            react_js_default().createElement("div", { className: "form-section" },
                ...
                brokers.length === 0 ? (react_js_default().createElement("div", { className: "warning-message" },
                    ...
                    " Please configure at least one MQTT broker in the Broker Configuration tab before creating topic mappings.",
                    ...
                    react_js_default().createElement((react_js_default()).Fragment, null,
                        ...
                        brokers.map(broker => {
                            ...
                            return (react_js_default().createElement("div", { key: broker.id, className: "broker-mappings-group" },
                                ...
                                react_js_default().createElement("div", { className: "mappings-list" }, brokerMappings.map(renderMappingItem))));
                        }))) : (
                    ...
                    react_js_default().createElement("div", { className: "broker-mappings-group" },
                        ...
                        react_js_default().createElement("div", { className: "mappings-list" }, (mappingsByBroker[selectedBrokerId] || []).map(renderMappingItem)))))) : (react_js_default().createElement("p", { className: "no-mappings" }, "No custom topic mappings configured yet.")),
            ...
            react_js_default().createElement("div", { className: "form-section" },
                ...
                react_js_default().createElement("div", { className: "form-group" },
                    react_js_default().createElement("label", null, "Default Payload Fields"),
                    renderPayloadFields(payloadFields, 'default', updatePayloadCoreField, updatePayloadField)),
                react_js_default().createElement("div", { className: "form-group checkbox" },
                    react_js_default().createElement("label", null,
